...
class GetDetailBook:
	...
	def gdbi(self):
		...
		# 3.获取图书基本信息
		bookinfo_dict = {}
		datas = source.xpath("//div[@id='info']//text()").extract()
		print('datas:')
		print(datas)
		datas = [data.strip() for data in datas]
		datas = [data for data in datas if data != ""]
		...
		for data in datas:
			if u"作者" in data:
				...
				if u":" in data:
					bookinfo_dict["author"] = datas[datas.index(data) + 1]
				# print(datas[datas.index(data) + 1])
				elif u":" not in data:
					bookinfo_dict["author"] = datas[datas.index(data) + 2]
				# print(datas[datas.index(data) + 2])
			elif u"/" in data:
				if datas.index(data) < 4:
					bookinfo_dict["author"] += "/{}".format(datas[datas.index(data) + 1])
				# print(datas[datas.index(data) + 1])
		...
		full_intro = {}
		# 4.1 内容简介
		content_intro = source.xpath(
			"//div[@class='related_info']/h2/*[contains(text(),'内容简介')]/following::div[@id='link-report']/span[contains(@class,'hidden')]/*/div[contains(@class,'intro')]")
		content_intro = content_intro.xpath("string(.)").extract()
		print('content_intro:')
		print(content_intro)
		# 4.2 作者简介
		author_intro = source.xpath(
			"//div[@class='related_info']/h2/*[contains(text(),'作者简介')]/following::div[contains(@class,'indent')]/span[contains(@class,'hidden')]/div[contains(@class,'intro')]")
		author_intro = author_intro.xpath("string(.)").extract()
		print('author_intro:')
		print(author_intro)
		if content_intro:
			full_intro['content_intro'] = content_intro[0]
		if author_intro:
			full_intro['author_intro'] = author_intro[0]
		print('full_intro:')
		print(full_intro)
		log("提取书籍详情页图书相关介绍完成")
		# 5.获取目录（如有）
		catalog_info = source.xpath("//div[@id='{}']/text()".format(catalog_id)).extract()
		catalog_info = [x.replace('·', '') for x in catalog_info]  # 去除目录最后的省略号，不是点.是·
		catalog_info = [x.strip() for x in catalog_info if
		                x.strip() != '' and x.strip() != '(' and x.strip() != ')']  # 去除目录最后的括号
		print('catalog_info:')
		print(catalog_info)
		log("提取书籍详情页图书目录完成")
		# 6.获取标签
		book_tags = source.xpath("//div[@class='indent']/span/a/text()").extract()
		print('book_tags:')
		print(book_tags)
		log("提取书籍详情页图书标签完成")
		# 7.获取丛书信息（如有）
		series_intro = source.xpath("//div[@class='subject_show block5']")
		series_intro = series_intro.xpath('string(.)').extract()
		print('series_intro:')
		print(series_intro)
		log("提取书籍详情页图书所属丛书信息完成")
		# 8.获取豆瓣评分（如有）
		try:
			rating_num = source.xpath("//strong[contains(@class,'rating_num')]/text()").extract_first().strip()
			print('rating_num:')
			print(rating_num)
		except:
			rating_num = '暂无评分'
		...
		# 9.获取评分人数（如有）
		try:
			rating_votes = source.xpath("//span[contains(@property,'votes')]/text()").extract_first().strip()
		except:
			rating_votes = '暂无人评价'
			print('rating_votes:')
			print(rating_votes)
		...
		if book_tags:
			result['tags'] = book_tags
		if series_intro:
			result['seriesintro'] = series_intro[0]
		if rating_num:
			result['ratenum'] = rating_num
		if rating_votes:
			result['ratevoters'] = rating_votes
			result['ratevoters2'] = rating_votes
		yield result
		# except Exception as e:
		# 	print('str(Exception):\t'.format(str(Exception)))
		# 	print('str(e):\t\t'.format(str(e)))
